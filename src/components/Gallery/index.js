...
class Gallery extends Component {
  state = {activatedId: imagesList[0].id}

  updateActiveId = id => {
    this.setState({activatedId: id})
  }

  render() {
    const {activatedId} = this.state
    const {imageUrl, imageAltText} = imagesList[activatedId]
    return (
        ...
        <div className="gallery-container">
          ...
          <ul className="thumbnail-con">
            {imagesList.map(eachImg => (
              <ThumbnailItem
                key={eachImg.id}
                imagesDetails={eachImg}
                updateActiveId={this.updateActiveId}
                isActive={eachImg.id !== activatedId}
              />
            ))}
          </ul>
        </div>
